...
class OrdersList extends Component {
  constructor(props) {
    ...
    this.attrs = ["name", "priority", "doable"];
    ...
    this.state = {
      ...
      sortItem: (a, b) => (a.priority > b.priority ? 1 : -1),
      items: [],
      lastChecked: null,
  ...
  async dataListRender() {
    try {
      const orders = await new Parse.Query(Parse.Object.extend("Order")).find();
        ...
        <div className="disable-text-selection">
          <Row>
            <Colxx xxs="12">
              ...
              <div className="mb-2">
                ...
                >
                  <IntlMessages id="orders.display-options" />{" "}
          ...
          <Row>
            {filteredOrderedItems.length > 0 ? (
              filteredOrderedItems.map(order => {
                if (this.state.displayMode === "imagelist") {
                  return (
                    <Colxx sm="6" lg="4" xl="3" className="mb-3" key={order.id}>
                      <ContextMenuTrigger
                        id="menu_id"
                        data={order.id}
                        collect={collect}
                      >
                        <Card
                          onClick={event =>
                            this.handleCheckChange(event, order.id)
                          }
                          className={classnames({
                            active: this.state.selectedItems.includes(order.id)
                          })}
                        >
                          <CardBody>
                            <Row>
                        ...
                        >
                          <div className="pl-2 d-flex flex-grow-1">
                            <div className="card-body d-flex flex-column flex-lg-row justify-content-between align-items-center">
                              ...
                              {!order.doable && (
                                <Label>Faltan materiales por satisfacer</Label>
                              )}
                            </div>
                            <div className="custom-control custom-checkbox pl-1 align-self-center pr-4">
                              <CustomInput
